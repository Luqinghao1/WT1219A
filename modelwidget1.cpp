...
#include "modelparameter.h" // [修改] 引入全局参数类
...
void ModelWidget1::onExportData() {
    ...
    QString path = QFileDialog::getSaveFileName(this, "导出CSV数据", defaultDir + "/Model1_Data.csv", "CSV Files (*.csv)");
...
void ModelWidget1::onExportImage() {
    ...
    QString path = QFileDialog::getSaveFileName(this, "导出图表图片", defaultDir + "/Model1_Chart.png", "PNG Image (*.png);;JPEG Image (*.jpg);;PDF Document (*.pdf)");
    if (path.isEmpty()) return;
    bool success = false;
...
// [修复] 修正缩进，消除 -Wmisleading-indentation 警告
