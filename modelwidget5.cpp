...
void ModelWidget5::initChart() {
    ...
    m_plotTitle = new QCPTextElement(m_plot, "复合页岩油储层试井曲线 (模型5: 变井储+定压边界)", QFont("SimHei", 14, QFont::Bold));
...
void ModelWidget5::setInputText(QLineEdit* edit, double value) { if(edit) edit->setText(QString::number(value, 'g', 8)); }
...
void ModelWidget5::onResetParameters() {
    ...
    setInputText(ui->cDEdit, 0.01); setInputText(ui->sEdit, 1.0);
...
void ModelWidget5::onChartSettings() {
...
void ModelWidget5::runCalculation() {
    ...
    emit calculationCompleted("Model5_Composite_ConstPres_VariableStorage", baseParams);
}
...
double ModelWidget5::scaled_besseli(int v, double x) { if (x < 0) x = -x; if (x > 600.0) return 1.0 / std::sqrt(2.0 * M_PI * x); return boost::math::cyl_bessel_i(v, x) * std::exp(-x); }
